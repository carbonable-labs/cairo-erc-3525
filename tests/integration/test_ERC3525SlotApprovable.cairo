...
@external
func test_e2e_3525_slot_approvable{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ) {
    alloc_locals;
    let (local instance) = contract_access.deployed();
    ...
    with instance {
        ...
        IERC3525.setApprovalForSlot(
            contract_address=instance,
            owner=account1,
            slot=Uint256(SLOT1, 0),
            operator=account4,
            approved=TRUE,
        );
        %{ stop_prank() %}

        // #2 (account1) -> #4 (account3) moved by operator account4
        %{ stop_prank = start_prank(caller_address=ids.account4, target_contract_address=ids.instance) %}
        let (_) = IERC3525.transferValueFrom(
            contract_address=instance,
            fromTokenId=Uint256(2, 0),
            toTokenId=Uint256(4, 0),
            to=0,
            value=Uint256(1, 0),
        );
        %{ stop_prank() %}
