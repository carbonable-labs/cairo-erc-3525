...
namespace assert_that {
    ...
    func token_supply_in_slot_is{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (returned_total_amount) = ERC3525SlotEnumerable.token_supply_in_slot(slot);
    ...
    func total_value_is{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        slot_felt: felt, expected_total_felt: felt
    ) {
        alloc_locals;
        let slot = Uint256(slot_felt, 0);
        let expected_total = Uint256(expected_total_felt, 0);
        let (returned_total) = ERC3525.total_value(slot);
