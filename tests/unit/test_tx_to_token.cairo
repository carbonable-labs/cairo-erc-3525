%lang starknet

from starkware.cairo.common.bool import TRUE, FALSE
from starkware.cairo.common.uint256 import Uint256, ALL_ONES, uint256_eq
from starkware.cairo.common.cairo_builtins import HashBuiltin

from openzeppelin.security.safemath.library import SafeUint256
from openzeppelin.token.erc721.enumerable.library import ERC721Enumerable
from openzeppelin.token.erc721.library import ERC721

from carbonable.erc3525.library import ERC3525
from tests.unit.library import assert_that, it

const NAME = 'Carbonable Project';
const SYMBOL = 'CP';
const DECIMALS = 18;
const ADMIN = 'admin';
const USER1 = 'user1';
const USER2 = 'user2';
const USER3 = 'user3';
const TOKN1 = 1;
const TOKN2 = 2;
const TOKN3 = 3;
const INVALID_ID = 666;
const SLOT1 = 'slot1';
const SLOT2 = 'slot2';

@external
func __setup__{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() {
    ERC721.initializer(NAME, SYMBOL);
    ERC721Enumerable.initializer();
    ERC3525.initializer(DECIMALS);
    ERC3525._mint(USER1, Uint256(TOKN1, 0), Uint256(SLOT1, 0), Uint256(42, 0));
    ERC3525._mint(USER2, Uint256(TOKN2, 0), Uint256(SLOT1, 0), Uint256(21, 0));
    ERC3525._mint(USER2, Uint256(TOKN3, 0), Uint256(SLOT2, 0), Uint256(21, 0));

    return ();
}

@view
func test_owner_can_transfer_value{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ) {
    let from_token_id = Uint256(TOKN1, 0);
    let to_token_id = Uint256(TOKN2, 0);
    let value = Uint256(10, 0);
    let caller = USER1;

    with caller {
        it.transfers_token(from_token_id, to_token_id, value);
    }

    return ();
}

@view
func test_approved_can_transfer_value{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() {
    alloc_locals;
    let from_token_id = Uint256(TOKN1, 0);
    let to_token_id = Uint256(TOKN2, 0);
    let value = Uint256(10, 0);
    let caller = USER3;

    %{
        stop_prank = start_prank(ids.USER1)
        expect_events({"name": "Approval"})
    %}
    ERC721.approve(USER3, from_token_id);
    %{ stop_prank() %}

    with caller {
        it.transfers_token(from_token_id, to_token_id, value);
    }

    return ();
}

@view
func test_value_approved_can_transfer{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() {
    alloc_locals;
    let from_token_id = Uint256(TOKN1, 0);
    let to_token_id = Uint256(TOKN2, 0);
    let value = Uint256(10, 0);
    let (approved_value) = SafeUint256.add(value, Uint256(1, 0));

    %{
        stop_prank = start_prank(ids.USER1)
        expect_events({"name": "ApprovalValue"})
    %}
    ERC3525.approve(from_token_id, USER3, approved_value);
    let (local old_allowance: Uint256) = ERC3525.allowance(from_token_id, USER3);
    %{ stop_prank() %}

    let caller = USER3;
    with caller {
        it.transfers_token(from_token_id, to_token_id, value);
    }

    let (allowance) = ERC3525.allowance(from_token_id, USER3);
    let (expected_allowance) = SafeUint256.sub_le(old_allowance, value);
    assert_that.allowance_is(from_token_id, USER3, expected_allowance);

    return ();
}

@view
func test_unlimited_value_approved_can_transfer{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() {
    alloc_locals;
    let from_token_id = Uint256(TOKN1, 0);
    let to_token_id = Uint256(TOKN2, 0);
    let value = Uint256(10, 0);
    let unlimited_approved_value = Uint256(ALL_ONES, ALL_ONES);

    %{
        stop_prank = start_prank(ids.USER1)
        expect_events({"name": "ApprovalValue"})
    %}
    ERC3525.approve(from_token_id, USER3, unlimited_approved_value);
    %{ stop_prank() %}
    let caller = USER3;
    with caller {
        it.transfers_token(from_token_id, to_token_id, value);
    }

    let (local new_allowance: Uint256) = ERC3525.allowance(from_token_id, USER3);
    let (is_unlimited) = uint256_eq(unlimited_approved_value, new_allowance);
    assert 1 = is_unlimited;

    return ();
}

@view
func test_approved_forall_can_transfer{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() {
    alloc_locals;
    let from_token_id = Uint256(TOKN1, 0);
    let to_token_id = Uint256(TOKN2, 0);
    let value = Uint256(10, 0);

    %{
        stop_prank = start_prank(ids.USER1)
        expect_events({"name": "ApprovalForAll"})
    %}
    ERC721.set_approval_for_all(USER3, TRUE);
    %{ stop_prank() %}

    let caller = USER3;
    with caller {
        it.transfers_token(from_token_id, to_token_id, value);
    }

    return ();
}

@view
func test_can_transfer_value_to_same_token_id{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() {
    alloc_locals;
    let from_token_id = Uint256(TOKN1, 0);
    let value: Uint256 = Uint256(10, 0);

    let caller = USER1;
    with caller {
        it.transfers_token_same_id(from_token_id, from_token_id, value);
    }

    return ();
}

// Reverts
@view
func test_anyone_cannot_transfer_value_token_to_token{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() {
    alloc_locals;
    let from_token_id = Uint256(TOKN1, 0);
    let to_token_id = Uint256(TOKN2, 0);
    let value = Uint256(10, 0);

    %{
        stop_prank = start_prank(ids.USER2) 
        expect_revert(error_message="ERC3525: insufficient allowance")
    %}
    ERC3525.transfer_from_token_id(from_token_id, to_token_id, value);
    %{ stop_prank() %}

    return ();
}

@view
func test_cannot_transfer_value_exceeding_balance{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() {
    alloc_locals;
    let from_token_id = Uint256(TOKN1, 0);
    let to_token_id = Uint256(TOKN2, 0);
    let (local balance: Uint256) = ERC3525.balance_of(from_token_id);
    let (value) = SafeUint256.add(Uint256(1, 0), balance);

    %{
        stop_prank = start_prank(ids.USER1) 
        expect_revert(error_message="ERC3525: transfer amount exceeds balance")
    %}
    ERC3525.transfer_from_token_id(from_token_id, to_token_id, value);
    %{ stop_prank() %}

    return ();
}

@view
func test_cannot_transfer_value_to_different_slot{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() {
    alloc_locals;
    let from_token_id = Uint256(TOKN1, 0);  // token in slot 1
    let to_token_id = Uint256(TOKN3, 0);  // token in slot 2
    let (local balance: Uint256) = ERC3525.balance_of(from_token_id);
    let value = Uint256(10, 0);

    %{
        stop_prank = start_prank(ids.USER1) 
        expect_revert(error_message="ERC3525: transfer slot mismatch")
    %}
    ERC3525.transfer_from_token_id(from_token_id, to_token_id, value);
    %{ stop_prank() %}

    return ();
}

@view
func test_cannot_transfer_value_to_nonexistent_token_id{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() {
    alloc_locals;
    let from_token_id = Uint256(TOKN1, 0);
    let to_token_id = Uint256(INVALID_ID, 0);
    let (local balance: Uint256) = ERC3525.balance_of(from_token_id);
    let value = Uint256(10, 0);

    %{
        stop_prank = start_prank(ids.USER1) 
        expect_revert(error_message="ERC3525: query for nonexistent token")
    %}
    ERC3525.transfer_from_token_id(from_token_id, to_token_id, value);
    %{ stop_prank() %}

    return ();
}

@view
func test_cannot_transfer_when_value_exceeds_allowance{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() {
    alloc_locals;
    let from_token_id = Uint256(TOKN1, 0);
    let to_token_id = Uint256(TOKN2, 0);
    let value = Uint256(10, 0);
    let (approved_value) = SafeUint256.sub_le(value, Uint256(1, 0));

    %{
        stop_prank = start_prank(ids.USER1)
        expect_events({"name": "ApprovalValue"})
    %}
    ERC3525.approve(from_token_id, USER3, approved_value);
    %{
        stop_prank()
        stop_prank = start_prank(ids.USER3); 
        expect_revert(error_message="ERC3525: insufficient allowance")
    %}
    ERC3525.transfer_from_token_id(from_token_id, to_token_id, value);
    %{ stop_prank() %}

    return ();
}

@view
func test_cannot_transfer_invalid_uint{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() {
    alloc_locals;
    let from_token_id = Uint256(TOKN1, 0);
    let to_token_id = Uint256(-1, -1);  // invalid Uint256
    let value = Uint256(10, 0);
    let (approved_value) = SafeUint256.sub_le(value, Uint256(1, 0));

    %{
        stop_prank = start_prank(ids.USER1); 
        expect_revert(error_message="ERC3525: value is not a valid Uint256")
    %}
    ERC3525.transfer_from(from_token_id, to_token_id, 0, value);
    %{ stop_prank() %}

    return ();
}

@view
func test_cannot_transfer_to_zero_id{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() {
    alloc_locals;
    let from_token_id = Uint256(TOKN1, 0);
    let to_token_id = Uint256(0, 0);  // zero token id
    let value = Uint256(10, 0);

    %{
        stop_prank = start_prank(ids.USER1); 
        expect_revert(error_message="ERC3525: cannot transfer token zero or to zero address")
    %}
    ERC3525.transfer_from(from_token_id, to_token_id, 0, value);
    %{ stop_prank() %}

    return ();
}

@view
func test_cannot_transfer_from_zero_id{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() {
    alloc_locals;
    let from_token_id = Uint256(0, 0);  // zero token
    let to_token_id = Uint256(TOKN2, 0);
    let value = Uint256(10, 0);

    %{
        stop_prank = start_prank(ids.USER1); 
        expect_revert(error_message="ERC3525: token_id is zero")
    %}
    ERC3525.transfer_from(from_token_id, to_token_id, 0, value);
    %{ stop_prank() %}

    return ();
}

// TODO test_transfer_value_token_to_token
// helpers:
//   it_transfers
//   it_emit
//   it.does not adjust owner balances
//   does not change owners
//   does not change slots
//   changes allowance
//   sets allowance?
// contexts:
//   Success
//    when called by owner
//    when called by token approved
//    when called by value approved
//    when called by unlimited value approved
//    when called by operator
//    when sent to the from token*
//   Revert
//    when transfer value exceeds balance
//    when tx to token with different slot
//    when invalid token ids
//    when anyone calls
//    when value exceeds allowance
//  Receiver
//   to non receiver contract
//   to valid receiver contract
//   to receiver contract unexpected value
//   to receiver contract that reverts (subcases? panic?)

// test transfer value from token to address
//  it.adjusts balance
//  it.transfer value
//  it emits Transfer/SlotChanged/TransferValue
//  it does not adjust owner of from
//  it does not adjust slots
// Success
//  (mod) when sent to the owner
// Reverts
//  transfer to the zero address

// test approve value
//  it clears allowance
//  it sets allowance
//  it emits ApprovalValue
// Context:
//  Success
//   when clearing allowance
//   when token transferred
//   when approving non-zero address
//     with no prior allowance
//     with prior allowance same addr
//     with prior allowance diff addr
//     when sender is value approved
//  Reverts
//    when approving owner
//    when not token_id owner (anyone)
//    when caller is approved individual
//    when token_id does not exist
//    when called by zero address

// _mint reverts with token_id 0
// mint emits TrasnferValue etc
// _mint_value reverts with non_existent token_id
//   emits TransferValue
//   adjust token balance
//   does not adjust token owner and slot
//   does not adjust owner balance

// _burn ?
